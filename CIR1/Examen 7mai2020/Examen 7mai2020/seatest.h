...
// Globals : Added by @PRI
extern char s_seatest[];
...
#define assert_n_array_equal(expected, actual, n) do { size_t seatest_count; for(seatest_count=0; seatest_count<n; seatest_count++) { /*char s_seatest[SEATEST_PRINT_BUFFER_SIZE];*/ sprintf_s(s_seatest,SEATEST_PRINT_BUFFER_SIZE,"Expected %d to be %d at position %d", actual[seatest_count], expected[seatest_count], (int)seatest_count); seatest_simple_test_result((expected[seatest_count] == actual[seatest_count]), s_seatest, __FUNCTION__, __LINE__);} } while (0)

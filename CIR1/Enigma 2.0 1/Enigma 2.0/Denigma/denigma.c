...
char lettres;
int x, y, z, n, tmp1, tmp2, tmp3, lettre, count = 1, i = 0, tab[30];


void calcul()
{
	if (n % 2 == 0) { lettre = lettre - 4 * count - 2 * z - x * x - x - 2 * y - x * y * z * i; } //calcul pair
	else { lettre = lettre - 4 * count - z - z * z - 2 * x - 2 * y - x * y * z * i; } //calcul impair
	while (lettre < 1)
	{
		lettre += 26;
	}
